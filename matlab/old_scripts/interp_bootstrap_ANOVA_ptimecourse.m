% Does the same thing as 'interp_bootstrap_ANOVA.m' but generates a time course of p-values. Saved
% as text files that can be loaded and plotted

% WARNING: this uses a TON of memory and can crash inferior systems. Can be especially slow when
...
% Define number of subject in a single bootstrap sample;
nsamp = 7;
...
% Define minimum timepoint for analysis (1 = beginning)
min_O = 2;

% Define maximum timepoint for analysis (ntp = end)
max_O = ntp;
...
% Find divergence point for each cluster, loop anova on moving xi cluster arrays
...
while a <= nclust
    while xi ~=0 && xi <15000
        ptc{a}(iter,xi) = anova1([Cond_array{a}{1:ncond}],[],'off');
        xi = xi + 1;
        clear Cond_array{a}
        while rgn <= nrgn_clust{a}
            while b <= ncond
...
save('ptc_500_1','ptc{1}','-ASCII');
save('ptc_500_2','ptc{2}','-ASCII');
save('ptc_500_3','ptc{3}','-ASCII');
save('ptc_500_4','ptc{4}','-ASCII');
