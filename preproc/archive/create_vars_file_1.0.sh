...
echo "----------------------------------------------------------------"
funcSeries=( $(editprompt "First functional series number: " 5) )
echo ""
isDone=false
while [ $isDone == "false" ]; do
	funcSeries=( ${funcSeries[*]} $(editprompt "Next Functional Series: " 6))
	echo ""
...
for (( i=0; i<${#funcSeries[@]}; i++ )); do
	funcLabel=( ${funcLabel[*]} $(editprompt "Label for Functional Series ${funcSeries[$i]}: " run1) )
done
echo ""

echo "----------------------------------------------------------------"
echo " Image Dimensions (standard X and Y is 64x64)"
echo "  (standard slice number is 38 for 2.0s TR, and 29 for 1.5s TR)"
echo "----------------------------------------------------------------"
