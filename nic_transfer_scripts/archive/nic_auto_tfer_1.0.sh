...
# Revision 1.1: jt
# 03.05.2013
#    Added a variable for transfer local target directory to make
...
# Check if we have new dirs to transfer.
nNew=$(ssh wheeler@136.142.36.60 "find /home/wheeler/studies/ -maxdepth 1 -mtime -1 -type d | awk 'FNR>1' | wc -l")
...
if [ $nNew -gt 0 ]; then
	...
	for file in $(ssh wheeler@136.142.36.60 "find /home/wheeler/studies/ -maxdepth 1 -mtime -1 -type d | awk 'FNR>1'"); do
	  echo ${file} >>${LOGFILE}
	  rsync -rlptyD --progress --partial --rsh="ssh -l wheeler" 136.142.36.60:${file} $transferDir >>${LOGFILE}
	done
