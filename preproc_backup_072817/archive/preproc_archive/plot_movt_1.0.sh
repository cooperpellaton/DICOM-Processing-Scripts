...
# Check for data file (output from preproc 1.3 [to be released] in QA dir)
...
while [ $datafileExists == false ]; do
	...
	pushd $inpath/movement || { echo "movement dir not found, exiting..."; exit 1; }
		...
		if [ $bfiles -gt 0 ]; then
			nbfiles=`find ${subid}_b*_xr3d.${extension} -type f | wc -l`
...
# compute max, min, mean for each movement domain (output array is [mean min max])
...
ymin=${dxstats[1]}	# start somewhere logical
...
for domain in `echo ${mins[*]}`; do
	mintest=$( echo "scale=3; $ymin > $domain " | bc) # pipe comparison to bc (outputs 1 if true)
	if [ $mintest -eq 1 ]; then
		ymin=$domain				# if true, this is the minimum
	fi
	unset mintest					# not needed any more
done
unset domain
unset mins
ymin=`echo $ymin | cut -d "." -f 1`; ymin=$((ymin-1)) # Round value upward to nearest integer
...
datafileBase=`basename $datafile`	#Clean up datafile name for plot title


# Also plot mean somewhere in the here doc.
# What about rms and sd?

# gnuplot script (-persist keeps graph open when here block is done):
